...
is feature-complete (390 tests passing) and the deploy entry point
(`python -m deploy.lipmm_run --event-ticker ...`) is ready for first
live use against any Kalshi event. Next planned deployment is
KXISMPMI-26MAY (8 active LIP strikes confirmed via the new
`/incentive_programs` integration).
...
pytest                       # run all tests (~1100; lipmm core: 390)
...
- **lipmm framework feature-complete.** 9 phases of control plane
  shipped: state + JWT auth + pause/kill/knobs (1), manual orders +
  side locks (2), WebSocket fan-out + multi-tab + optimistic
  concurrency (3), htmx+Jinja dashboard (4), disk retention manager
  (5), positions/orders panel + surgical cancel (6), manual theo
  overrides via dashboard (7), Kalshi /incentive_programs surface
  (8), single-command deploy entry point (9).
- **390 lipmm tests passing.** ~1100 total in the repo (the rest is
  soy-era).
- **Soy bot paused.** Not currently running. The new entry point
  `deploy/lipmm_run.py` replaces `deploy/lip_mode.py` etc.
- **Dashboard surface available**: kill/arm flow, pause global/ticker/side,
  ...
  routing, theo overrides with two-step confirm, positions panel
  (qty + Yes/No badge + realized PnL coloring), resting-orders panel
  (per-row cancel button), balance pill + total-PnL pill, decision
  feed, LIP incentives panel with countdown, presence pill (multi-tab),
  optimistic concurrency on every mutation.
- **Deploy is ready, hasn't been smoked end-to-end yet.** The script
  imports clean and constructs everything; first live run is the
  operator's first integration test.
...
- `lipmm/quoting/` — DefaultLIPQuoting, StickyDefenseQuoting
...
- `lipmm/incentives/` — IncentiveProgram, KalshiIncentiveProvider, IncentiveCache (hourly refresh, fault-tolerant)
...
1. **First live deploy** — run `python -m deploy.lipmm_run --event-ticker KXISMPMI-26MAY` end-to-end. Watch for unexpected errors. Set theo overrides via dashboard for a few strikes.
